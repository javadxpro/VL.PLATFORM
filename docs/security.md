...
- هویت عمومی کاربر `users.vl_id` است (`VL-XXXX-XXXX`، مهاجرت 0011): **ترتیبی
  نیست**، پس «همهٔ کاربران را با افزایش id بردار» امکان‌پذیر نیست؛ **تغییرپذیر
  نیست** (هیچ route‌ای آن را نمی‌نویسد، تستش در `tests/test_vlid.py` است) و
  **قابل جست‌وجو نیست** — هنوز endpoint‌ای برای lookup با آن وجود ندارد
  (تصمیم باز در `docs/ROADMAP.md` §۵). این یک اعتبارنامه نیست: برای اشاره‌کردن به
  یک حساب است، نه برای اثبات مالکیت آن.
